...
import com.yibei.system.mapper.SysDictTypeMapper;
...
public class SysDictTypeServiceImpl extends ServicePlusImpl<SysDictTypeMapper, SysDictType, SysDictType> implements ISysDictTypeService {
	...
	public void loadingDictCache() {
		...
		for (SysDictType dictType : dictTypeList) {
			List<SysDictData> dictDatas = dictDataList.stream().filter(s -> s.getDictType().equals(dictType.getDictType())).collect(Collectors.toList());
			//List<SysDictData> dictDatas = dictDataMapper.selectDictDataByType(dictType.getDictType());
			DictUtils.setDictCache(dictType.getDictType(), dictDatas);
		}
		log.info("初始化字典到缓存-完成");
	}
